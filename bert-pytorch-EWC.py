...
#读入数据集
data_path = '/home/xym/ljw/bert/English_2/'
...
#装载模型
model = BertForSequenceClassification.from_pretrained('/home/xym/ljw/bert/EWC435_French_result/', num_labels=2)
model.to(device)
...
#装载F1重要度矩阵
precision_matrices = np.load('F1.npy', allow_pickle='TRUE')

#训练部分
def trainer(train_dataloader, validation_dataloader, test_dataloader, model, optimizer, scheduler, precision_matrices, weight_decay_ewc):
  #训练部分
  train_loss_set = []
  epochs = 3
  ...
  #输出保存训练后的模型
  output_dir2 = '/home/xym/ljw/bert/EWC435_English_French_result/'
...
#训练
weight_decay_ewc = 435
result = trainer(train_dataloader, validation_dataloader, test_dataloader, model, optimizer, scheduler, precision_matrices, weight_decay_ewc)
...
precision_matrices2 = {} #重要度
for n, p in params.items():
  precision_matrices2[n] = p.clone().detach().fill_(0) 
...
for step,batch in enumerate(train_dataloader):
  ...
  for n, p in model.named_parameters():                         
      precision_matrices2[n].data += p.grad.data ** 2 / len(train_dataloader)
        ########### 计算对数概率的导数，然后反向传播计算梯度，以梯度的平方作为重要度 ########

np.save('F2.npy', precision_matrices2)
